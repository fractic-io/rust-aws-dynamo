...
pub(crate) fn generate_id_with_options<T: DynamoObject>(
    ...
    let terminal_segment = match T::id_logic() {
        IdLogic::Phantom => Err(CriticalError::new(concat!(
            "IDs for IdLogic::Phantom must be constructed manually; ",
            "phantom objects cannot be persisted",
        ))),
        IdLogic::Uuid => Ok(format!("{}#{}", T::id_label(), new_uuid_value())),
        IdLogic::Timestamp => {
            let timestamp = options
                .timestamp_millis
                .unwrap_or_else(|| chrono::Utc::now().timestamp_millis());
            Ok(format!("{}#{}", T::id_label(), timestamp_value(timestamp)?))
        }
        IdLogic::Singleton | IdLogic::SingletonExt => Ok(format!("@{}", T::id_label())),
        IdLogic::IndexedSingleton(key) | IdLogic::IndexedSingletonExt(key) => {
            Ok(format!("@{}[{}]", T::id_label(), key(data)))
        }
        IdLogic::BatchOptimized { .. } => Err(CriticalError::new(concat!(
