...
use crate::{
    ...
    schema::{
        id_calculations::{extdata_base_sk, generate_pk_sk, get_object_type, get_pk_sk_from_map},
        parsing::{
    ...
    util::{
        ...
        extdata::{collapse_query_results, matches_conditions, ExtDataMetadata, ExtDataStore},
...
pub(crate) use extdata::EXTDATA_RESERVED_KEY;
...
impl DynamoUtil {
    ...
    fn extdata(&self) -> ExtDataStore<'_> {
        ExtDataStore::new(self)
    }
    ...
    pub async fn query_generic(
        ...
        let (id, match_type) = ExtDataStore::normalize_query(id, match_type)?;
        ...
        let items = collapse_query_results(
            response
                .into_iter()
    ...
    pub async fn item_exists(&self, id: PkSk) -> Result<bool, ServerError> {
        if extdata_base_sk(&id.sk).is_some() {
            return self.extdata().item_exists(&id).await;
        }
        let key = collection! {
            "pk".to_string() => AttributeValue::S(id.pk),
            "sk".to_string() => AttributeValue::S(id.sk),
        ...
        if matches!(T::id_logic(), IdLogic::ExtData) {
            self.extdata()
                .replace_object::<T>(
                    ...
                    &data,
                    ExtDataMetadata::new(Some(now.clone()), Some(now), sort, ttl),
                )
                .await?;
    ...
    pub async fn update_item<T: DynamoObject>(&self, object: &T) -> Result<(), ServerError> {
        ...
        if matches!(T::id_logic(), IdLogic::ExtData) {
            ...
            let existing = self
                .get_item::<T>(ExtDataStore::canonical_id(object.id())?)
                .await?
            ...
            return self
                ...
                .replace_object::<T>(
                    ...
                    object.data(),
                    ExtDataMetadata::new(
                        existing.auto_fields().created_at.clone(),
                        Some(Timestamp::now()),
                        existing.auto_fields().sort,
                        existing.auto_fields().ttl,
                    ),
                )
                .await;
        }
    ...
    pub async fn update_item_transaction<T: DynamoObject>(
        ...
        if matches!(T::id_logic(), IdLogic::ExtData) {
            ...
            let metadata = match object_before {
                Some(existing) => ExtDataMetadata::new(
                    existing.auto_fields().created_at.clone(),
                    Some(Timestamp::now()),
                    existing.auto_fields().sort,
                    existing.auto_fields().ttl,
                ),
                None => ExtDataMetadata::new(None, Some(Timestamp::now()), None, None),
            };
            self.extdata()
                .replace_object::<T>(&logical_id, &data_after, metadata)
                .await?;
    ...
    pub async fn update_item_with_conditions<T: DynamoObject>(
        ...
        if matches!(T::id_logic(), IdLogic::ExtData) {
            ...
            let existing = self
                ...
                .await?
                .ok_or_else(DynamoNotFound::new)?;
            if !matches_conditions(existing.data(), &conditions)? {
                return Err(DynamoInvalidOperation::new(
    ...
    pub async fn delete_item<T: DynamoObject>(&self, id: PkSk) -> Result<(), ServerError> {
        ...
        validate_id::<T>(&id)?;
        if matches!(T::id_logic(), IdLogic::ExtData) {
            return self.extdata().delete_object(&id).await;
        }
        let key = collection! {
            "pk".to_string() => AttributeValue::S(id.pk),
            "sk".to_string() => AttributeValue::S(id.sk),
    ...
    pub async fn batch_delete_item<T: DynamoObject>(
        ...
        if matches!(T::id_logic(), IdLogic::ExtData) {
            let raw_ids = self.extdata().raw_ids_for_many(keys).await?;
            return self.raw_batch_delete_ids(raw_ids).await;
        }
